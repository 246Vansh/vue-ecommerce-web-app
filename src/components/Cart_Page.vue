...
const addToCheckout = () => {
  if (state?.cart?.length) {
    ...
    showToast(toast, "error", "Product not available.");
  }
};

const subtotal = computed(() =>
  state.cart.reduce((total, product) => total + calculateDiscountedPrice(product.price, product.discountPercentage) * product.quantity, 0),
);
      ...
      <form class="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
        <section aria-labelledby="cart-heading" class="lg:col-span-7">
          ...
          <ul role="list" class="divide-y divide-gray-200 border-b border-t border-gray-200">
            <li v-for="(product, productIdx) in state.cart" :key="product.id" class="flex py-6 sm:py-10">
              <div class="shrink-0">
                <img  :src="product.images && product.images[0] ? product.images[0] : 'https://via.placeholder.com/150'" 
                      :alt="product.imageAlt" class="size-24 rounded-md object-contain sm:size-48" />
              </div>
              ...
              <div class="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                <div class="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                  <div>
                    <div class="flex justify-between">
                      <h3 class="text-sm">
                        <p class="font-medium text-gray-700 hover:text-gray-800">{{ product.title }}</p>
                      </h3>
                    </div>
                    <!--
                ...
                <p class="mt-4 flex space-x-2 text-sm text-gray-700">
                  <CheckIcon v-if="product.availabilityStatus" class="size-5 shrink-0 text-green-500" aria-hidden="true" />
                  <ClockIcon v-else class="size-5 shrink-0 text-gray-300" aria-hidden="true" />
                  <span>{{ product.availabilityStatus ? 'In stock' : `Ships in ${product.shippingInformation}` }}</span>
                </p>
              </div>
            </li>
          </ul>
        ...
        <section aria-labelledby="summary-heading" class="mt-16 rounded-lg bg-gray-50 px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8">
          ...
          <div class="mt-6">
            <button @click="addToCheckout" type="button" class="w-full rounded-md border border-transparent bg-indigo-600 px-4 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50">Checkout</button>
          </div>
        </section>
      </form>
