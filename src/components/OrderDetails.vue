...
onMounted(() => {
  ...
  purchasedProducts.value.forEach((products) => {
  products.step = Math.floor(Math.random() * 4)})
...
const subtotal = computed(() =>
  purchasedProducts.value.reduce((total, products) => total + calculateDiscountedPrice(products.price, products.discountPercentage) * products.quantity, 0),
);
...
const shipping = computed(() => {
  ...
  return subtotal.value < 100 ? 4.99 : 0.00;
});

const tax = computed(() => subtotal.value * state.taxRate);
const orderTotal = computed(() => subtotal.value + shipping.value + tax.value);
          ...
          <div class="mx-auto flex h-10 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <div class="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
              <div v-if="user">
                ...
                <button @click="handleLogout" class="text-sm font-medium text-white hover:text-gray-800">Logout</button>
                </div>
              </div>
              <div v-else>
                <div class="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                ...
                <span class="h-6 w-px bg-gray-200" aria-hidden="true" />
                <button @click="handleSignup" class="text-sm font-medium text-white-700 hover:text-gray-800">Create account</button>
                </div>
              </div>
            </div>
          </div>
              ...
              <div class="flex h-16 items-center justify-between">
                ...
                <div class="flex flex-1 items-center justify-end">
                  <div class="flex items-center lg:ml-8">
                    ...
                    <div class="flow-root">
                      <button @click="handleCartBag" class="group -m-2 flex items-center p-2">
    ...
    <main class="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
      ...
      <div class="mt-2 border-b border-gray-200 pb-5 text-sm sm:flex sm:justify-between">
        <dl class="flex">
          ...
          <dd class="font-medium text-gray-900"><time datetime="2021-03-22">{{ formattedDate }}</time></dd>
        </dl>
      ...
      <section aria-labelledby="products-heading" class="mt-8">
        ...
        <div class="space-y-24">
          <div v-for="product in purchasedProducts" :key="product.id" class="grid grid-cols-1 text-sm sm:grid-cols-12 sm:grid-rows-1 sm:gap-x-6 md:gap-x-8 lg:gap-x-8">
            <div class="sm:col-span-4 md:col-span-5 md:row-span-2 md:row-end-2">
              <img :src="product.images && product.images[0] ? product.images[0] : 'https://via.placeholder.com/150'"
                    ...
                    class="aspect-square w-full rounded-lg bg-gray-50 object-contain" />
            </div>
            ...
            <div class="sm:col-span-12 md:col-span-7">
              <dl class="grid grid-cols-1 gap-y-8 border-b border-gray-200 py-8 sm:grid-cols-2 sm:gap-x-6 sm:py-6 md:py-10">
                <div>
                  ...
                  <dd class="mt-3 text-gray-500">
                    <span class="block">{{ state.shippingInformation.House }}</span>
                    <span class="block">{{ state.shippingInformation.Address }}</span>
                    <span class="block">{{ state.shippingInformation.City }}, {{ state.shippingInformation.State }}, {{ state.shippingInformation.PostalCode }}</span>
                  </dd>
                </div>
                <div>
                  ...
                  <dd class="mt-3 space-y-3 text-gray-500">
                    <div>
                      <p>{{ state.contactInformation.Email }}</p>
                    </div>
                    <div>
                      <p>{{ state.shippingInformation.Phone }}</p>
                    </div>
              ...
              <p class="mt-6 font-medium text-gray-900 md:mt-10">
                Processing on <time>{{ formattedDate }}</time>
              </p>
              <div class="mt-6">
                ...
                <div class="mt-6 hidden grid-cols-4 font-medium text-gray-600 sm:grid">
                    <div :class="[product.step > 0 ? 'text-indigo-600' : '', 'text-center']">Order placed</div>
                    <div :class="[product.step > 1 ? 'text-indigo-600' : '', 'text-center']">Processing</div>
                    <div :class="[product.step > 2 ? 'text-indigo-600' : '', 'text-center']">Shipped</div>
                    <div :class="[product.step === 3 ? 'text-indigo-600' : '', 'text-right']">Delivered</div>
                </div>
                </div>
              </div>
            </div>
        ...
        <div class="rounded-lg bg-gray-50 px-6 py-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:px-0 lg:py-8">
          <dl class="grid grid-cols-1 gap-6 text-sm sm:grid-cols-2 md:gap-x-8 lg:col-span-5 lg:pl-8">
            ...
            <div >
              ...
              <dd class="mt-3 flex">
                ...
                <div class="ml-4">
                  <p class="text-gray-900">Ending with {{ state.paymentDetail.cardNumber.slice(-4) }}</p>
                  <p class="text-gray-600">Expires {{ state.paymentDetail.cardDate }}</p>
                </div>
              </dd>
            </div>
          </dl>
          ...
          <dl class="mt-8 divide-y divide-gray-200 text-sm lg:col-span-7 lg:mt-0 lg:pr-8">
            ...
            <div class="flex items-center justify-between py-4">
              ...
              <dd class="font-medium text-gray-900">${{ shipping.toFixed(2) }}</dd>
            </div>
            <div class="flex items-center justify-between py-4">
              <dt class="text-gray-600">Tax</dt>
              <dd class="font-medium text-gray-900">${{ tax.toFixed(2) }}</dd>
            </div>
            <div class="flex items-center justify-between pt-4">
              <dt class="font-medium text-gray-900">Order total</dt>
              <dd class="font-medium text-indigo-600">${{ orderTotal.toFixed(2) }}</dd>
            </div>
          </dl>
        </div>
      </section>
